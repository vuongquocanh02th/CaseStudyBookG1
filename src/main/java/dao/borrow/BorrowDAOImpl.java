...
public class BorrowDAOImpl implements IBorrowDAO {
    private static final String SELECT_ALL_BORROWS = "SELECT b.ID, b.CustomerID, c.Name AS CustomerName, b.BorrowDate, b.ReturnDate, bd.BookID, bk.BookName " +
            "FROM Borrow b " +
            "JOIN Customer c ON b.CustomerID = c.ID " +
            "JOIN BorrowDetail bd ON b.ID = bd.BorrowID " +
            "JOIN Books bk ON bd.BookID = bk.ID";
    private static final String SELECT_BORROW_BY_ID = "SELECT b.ID, b.CustomerID, c.Name AS CustomerName, b.BorrowDate, b.ReturnDate, bd.BookID, bk.BookName " +
            "FROM Borrow b " +
            "JOIN Customer c ON b.CustomerID = c.ID " +
            "JOIN BorrowDetail bd ON b.ID = bd.BorrowID " +
            "JOIN Books bk ON bd.BookID = bk.ID " +
            "WHERE b.ID = ?";
    private static final String SELECT_BORROW_DETAILS_BY_BORROW_ID = "SELECT bd.ID, bd.BorrowID, bd.BookID, bk.BookName, bd.BorrowDate, bd.ReturnDate, bd.ReturnStatus " +
    ...
    public List<Borrow> findAll() {
        ...
        try (Connection connection = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                borrows.add(new Borrow(id, customerId, customerName, borrowDate, returnDate, bookId, bookName));
            }
    ...
    public Borrow findById(int id) {
        ...
        try (Connection connection = DBConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                int bookId = rs.getInt("BookID");
                String bookName = rs.getString("BookName");
                borrow = new Borrow(id, customerId, customerName, borrowDate, returnDate, bookId, bookName);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void save(Borrow borrow) {
        String INSERT_BORROW = "INSERT INTO Borrow (CustomerID, BorrowDate, ReturnDate) VALUES (?, ?, ?)";
        try (Connection connection = DBConnection.getConnection();
    ...
    public void update(Borrow borrow) {
        String UPDATE_BORROW = "UPDATE Borrow SET CustomerID = ?, BorrowDate = ?, ReturnDate = ? WHERE ID = ?";
        try (Connection connection = DBConnection.getConnection();
    ...
    public void delete(int id) {
        String DELETE_BORROW = "DELETE FROM Borrow WHERE ID = ?";
        try (Connection connection = DBConnection.getConnection();
